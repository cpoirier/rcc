   ...
   class State
    ...
    protected
      ...
      def enumerate_transitions( constraint = nil )
         ...
         # lexical_transitions = CharacterRange.new()
         # lexical_items       = []
         # 
         # @items.each do |item|
         #    next if item.complete?
         ...
         #    item.leaders().each do |leader|
         #       if leader.refers_to_character? then
         #          lexical_transitions ^= leader
         #          lexical_items       << item
         #       else
         #          enumeration[leader.name] = [] unless enumeration.member?(leader.name)
         #          enumeration[leader.name] << item.shift
         #       end
         #    end
         # end
         # 
         # 
         # #
         # # Next, separate out the CharacterRanges into unique sets and direct traffic along each vector.
         #       
         # lexical_transitions.each_range do |range|
         #    vector = Plan::CharacterRange.new(range)
         #    enumeration[vector] = []
         #    lexical_items.each do |item|
         #       enumeration[vector] << item.shift if item.leader.overlaps?(vector)
         #    end
         # end
...
# #
# # close_lexer()
# #  - expands the state to include everything needed to read our complete items' determinants
# #  - as lexes are always longest match, we don't add anything that would compete with existing reads
# 
# def close_lexer()
#    @lexer_closed = true
#    assert( @queue.empty?, "shouldn't the queue be empty?" )
# 
#    natural_shifts = CharacterRange.new()
#    @items.each do |item|
#       next unless item.lexical?
#       next unless item.leader.is_a?(CharacterRange)
#       natural_shifts.add( item.leader )
#    end
#    
#    puts natural_shifts if @number == 19
# 
#    unless context_free?
#       @queue = @items.select{|item| item.complete? and item.syntactic?}
#       until @queue.empty?
#          item = @queue.shift
#          next unless item.syntactic?
# 
#          warn_nyi( "shouldn't we be adding unique discard from complete contexts?" )
#          item.determinants.each do |determinant|
#             next unless determinant.producible?
#             
#             puts determinant.name if @number == 19
#             if @master_plan.production_sets.member?(determinant) then
#                complete_set = @master_plan.production_sets[determinant]
#                usable_set   = ProductionSet.new()
#                
#                complete_set.productions.each do |production|
#                   occluded = production.occlude( natural_shifts, @master_plan )
#                   usable_set << occluded unless occluded.nil?
#                end
#                
#                add_productions( usable_set, item )
#             else
#                nyi "error handling for missing reference name [#{determinant}]"
#             end
#          end
#       end
#    end
...
# 
#    @items.each do |item|
#       @lexible[item.production.name] = true if item.production.lexical?
#    end
#    
#    #
#    # Before leaving, we can free up the Item index, as we are now closed to new Items.
#    
#    @item_index = nil
# end
# 
...
# 
# def enumerate_lexical_transitions()
#    assert( @lexer_closed, "you must call close_lexer() on State #{@number} before enumerating the lexical transitions" )
#    
#    #
#    # First, map out the character ranges.
#    
#    enumeration   = Util::OrderedHash.new()
#    lexical_items = []
#    lexical_transitions = Plan::CharacterRange.new()
#    
#    @items.each do |item|
#       next if item.complete?
#       
#       item.leaders(false).each do |leader|
#          next unless leader.refers_to_character?
#          lexical_transitions ^= leader
#          lexical_items << item
#       end
#    end
...
# last_is_longest = false
# vectors = CharacterRange.new()
# tokenize_vectors.each do |production, range|
#    vectors ^= range
#    if production.length > attempt_span then
#       attempt_span = production.length
#       last_is_longest = true
...
# end
#    
# sets = Util::SparseArray.new()
# vectors.each_range do |range|
#    set = []
#    sets[range] = set
#    tokenize_vectors.each do |production, production_range|
#       set << production if production_range.overlaps?(range)
#    end
# end
#    
# sets.each do |range, set|
#    if set.length == 1 then
#       @literal_actions[range] = Actions::Tokenize.new( set[0] )
#    else
#    
#       #
#       # Sort the set into a reasonable order.  We prefer long matches over short ones.  If they are
...
#             if a.symbols[-1].is_a?(Plan::Symbol) then 
#                -1
#             elsif b.symbols[-1].is_a?(Plan::Symbol) then
#                1
#             else
#                a.symbols[-1].length <=> b.symbols[-1].length
#             end
#          else
#             b.length <=> a.length 
#          end
#       end
#    
#       @literal_actions[range] = Actions::Attempt.new( set.collect{|p| Actions::Tokenize.new(p)}, attempt_span, last_is_longest )
#    end
# end
# 
# 
...
# 
# #
# # Step 4: Next in decreasing priority are group actions.  We fill in around any existing
# 
# 
# #
# # For Tokenize and Group determinants, we use follow characters.  We always take Group over Tokenize (again,
# # because we want longest matches), and if there are two or more equally good options, we enable backtracking, 
# # and try the most specific option first.
#    
# unless complete_items.empty?
# 
#    tokenize_vectors = CharacterRange.new()
#    tokenize_vectors = Util::OrderedHash.new()
#    complete_items.each do |item|
#       determinants = nil
#       duration = Time.measure do
#          determinants = item.determinants()
#       end
# 
...
#       
#       remaining = determinants - covered
#       unless remaining.empty?
#          tokenize_vectors[item.production] = CharacterRange.new() unless tokenize_vectors.member?(item.production)
#          tokenize_vectors[item.production].add(remaining)
#       end
#    end
#       
#    unless tokenize_vectors.empty?
#       if tokenize_vectors.length == 1 then
#          tokenize_vectors.each do |production, range|
#             @literal_actions[range] = Actions::Tokenize.new( production )
#          end
#       else
#          attempt_span = 0
#          last_is_longest = false
